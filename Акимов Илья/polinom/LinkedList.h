...
template<typename T>
TLinkedList<T>::TLinkedList() : length(0), first(nullptr), last(nullptr) {}

template<typename T>
TLinkedList<T>::TLinkedList(const TLinkedList& other) : length(other.length), first(nullptr), last(nullptr)
{
	if (!other.first)
		return;
